...
int main(int ac, char **av)
{
    ...
    float rayon = (atoi(av[8]) * M_PI) / 180;
    float racine_1;
    float racine_2;
    float delta;
    float a_calc;
    ...
    if (deter == 1) {
        ...
        a_calc = pow(argv5, 2) + pow(argv6, 2) - (pow(argv7, 2) / pow(tan(rayon), 2));
        b_calc = (2 * argv2 * argv5) + (2 * argv3 * argv6) - (2 * argv4 * argv7 / pow(tan(rayon), 2));
        c_calc = pow(argv2, 2) + pow(argv3, 2) - pow(argv4, 2) / pow(tan(rayon), 2);
    }
    else 
        exit (84);
